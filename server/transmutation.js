/**
 * DepEd K-12 Transmutation Table Logic
 * This mimics the official DepEd Order No. 8, s. 2015 transmutation scale.
 */
...
const transmute = (initialGrade) => {
    const numericGrade = parseFloat(initialGrade);
    const match = transmutationTable.find(row => numericGrade >= row.min && numericGrade <= row.max);
    return match ? match.grade : 60; // Default to 60 if somehow out of range
};
